...
CLASS zcl_dutils_oea_iwpr_env_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_dutils_oea_iwpr_env_srv IMPLEMENTATION.
  ...
  METHOD read_generated_objects.
    SELECT trobj_type AS tadir_type,
      ...
      WHERE project = @segw_project
        AND ( trobj_type = @zif_dutils_c_tadir_type=>class OR
              trobj_type = @zif_dutils_c_tadir_type=>interface )
  ...
  METHOD add_sadl_objects.
    ...
    LOOP AT sadl_definition-data_sources ASSIGNING FIELD-SYMBOL(<data_source>) WHERE type = 'CDS'.
      APPEND zcl_dutils_oea_factory=>create_used_object(
        ...
        external_type = zif_dutils_c_tadir_type=>data_definition ) TO used_objects.
    ENDLOOP.
  ENDMETHOD.
