...
CLASS zcl_dutils_oea_sicf_env_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_dutils_oea_sicf_env_srv IMPLEMENTATION.
  ...
  METHOD zif_dutils_oea_env_service~determine_used_objects.
    ...
    LOOP AT service_info-handlertbl ASSIGNING FIELD-SYMBOL(<handler_info>).
      INSERT zcl_dutils_oea_factory=>create_used_object(
        name          = CONV #( <handler_info>-icfhandler )
        external_type = zif_dutils_c_tadir_type=>class ) INTO TABLE result.
    ENDLOOP.
  ENDMETHOD.
